...
func (g *projectGenerator) generateFile(file File) {
	...
	contractSource, ok := g.generatedContractSource(file)
	if !ok {
		return
	}
	...
	if contractSource != nil {
		contractPath = stem + "_contract_tue.go"
		g.files = append(g.files, GeneratedFile{Path: contractPath, Source: contractSource})
	}
...
func (g *projectGenerator) generatedScriptSource(file File) ([]byte, bool) {
	...
	if file.Script.Component.ContractType != "" {
		if !injectContractField(parsed, file.Script.Component) {
...
func injectContractField(file *ast.File, component *script.Component) bool {
	for _, declaration := range file.Decls {
		...
		for _, spec := range general.Specs {
			...
			field := &ast.Field{
				Names: []*ast.Ident{ast.NewIdent(script.ContractFieldName())},
				Type:  ast.NewIdent(component.ContractType),
			}
...
func (g *projectGenerator) generatedContractSource(file File) ([]byte, bool) {
	component := file.Script.Component
	if component.ContractType == "" {
		return nil, true
	}

	generated := jen.NewFile(file.Script.PackageName)
	generated.HeaderComment("Code generated by tue; DO NOT EDIT.")
	imports := contractImports(file.Script.Imports)
	for alias, path := range imports {
		generated.ImportName(path, alias)
	}
	fields := make([]jen.Code, 0, len(component.Props)+len(component.Events)+len(component.ContractStates))
	for _, prop := range component.Props {
		valueType, ok := renderContractTypeExpression(prop.Type, imports)
		if !ok {
	...
	for _, event := range component.Events {
		callbackType, ok := renderContractTypeExpression(event.FunctionType(), imports)
		if !ok {
	...
	for _, state := range component.ContractStates {
		valueType, ok := renderContractTypeExpression(state.Type, imports)
		if !ok {
			g.add(filePath(file), fmt.Sprintf("component state %q has unsupported type %q", state.Name, state.Type), state.TypeSpan)
			return nil, false
		}
		fields = append(fields, jen.Id(script.StateFieldName(state.GoName)).Qual(tueImportPath, "Ref").Types(valueType))
	}
	generated.Type().Id(component.ContractType).Struct(fields...)
	initialValues := make([]jen.Code, 0, len(component.ContractStates))
	for _, state := range component.ContractStates {
		valueType, _ := renderContractTypeExpression(state.Type, imports)
		initialValues = append(initialValues,
			jen.Id(script.StateFieldName(state.GoName)).Op(":").Qual(tueImportPath, "RefOf").Call(jen.Op("*").New(valueType)),
		)
	}
	generated.Func().Id(script.ContractConstructorName(component.Name)).Params().Id(component.ContractType).Block(
		jen.Return(jen.Id(component.ContractType).Values(initialValues...)),
	)

	for _, prop := range component.Props {
		valueType, _ := renderContractTypeExpression(prop.Type, imports)
		fieldName := script.PropFieldName(prop.GoName)
		presenceName := script.PropOKName(prop.GoName)
		presenceStatements := []jen.Code{
			jen.If(jen.Id("component").Op("==").Nil().Op("||").Id("component").Dot(script.ContractFieldName()).Dot(fieldName).Op("==").Nil()).Block(
				jen.Var().Id("zero").Add(valueType),
				jen.Return(jen.Id("zero"), jen.False()),
			),
			jen.List(jen.Id("value"), jen.Id("ok")).Op(":=").Id("component").Dot(script.ContractFieldName()).Dot(fieldName).Call(),
	...
	for _, event := range component.Events {
		...
		generated.Func().Params(jen.Id("component").Op("*").Id(component.Name)).Id(script.EventMethodName(event.GoName)).Params(parameters...).Bool().Block(
			jen.If(
				jen.Id("component").Op("==").Nil().Op("||").Id("component").Dot(script.ContractFieldName()).Dot(fieldName).Op("==").Nil(),
			).Block(jen.Return(jen.False())),
			jen.Id("component").Dot(script.ContractFieldName()).Dot(fieldName).Call(arguments...),
			jen.Return(jen.True()),
		)
	}
	for _, state := range component.ContractStates {
		...
		generated.Func().Params(jen.Id("component").Op("*").Id(component.Name)).Id(script.StateGetterName(state.GoName)).Params().Add(valueType).Block(
			...
			jen.Return(jen.Id("component").Dot(script.ContractFieldName()).Dot(fieldName).Dot("Get").Call()),
		)
		generated.Func().Params(jen.Id("component").Op("*").Id(component.Name)).Id(script.StateSetterName(state.GoName)).Params(jen.Id("value").Add(valueType)).Block(
			...
			jen.Id("component").Dot(script.ContractFieldName()).Dot(fieldName).Dot("Set").Call(jen.Id("value")),
		)
	}

	source, err := renderJenniferFile(generated)
	if err != nil {
		g.add(filePath(file), fmt.Sprintf("render generated component contract: %v", err), component.Span)
		return nil, false
	}
	return source, true
}

func contractImports(imports []script.Import) map[string]string {
...
func renderEventParameters(event script.Event, imports map[string]string) ([]jen.Code, []jen.Code, bool) {
	...
	for index, parameter := range event.Parameters {
		...
		typeCode, ok := renderContractTypeExpression(parameterType, imports)
		if !ok {
...
func (g *fileGenerator) generate(tree *gotemplate.Tree) {
	...
	if g.component.ContractType != "" {
		componentValues = append(componentValues,
			jen.Id(script.ContractFieldName()).Op(":").Id(script.ContractConstructorName(g.component.Name)).Call(),
		)
	}
	g.file.Func().Id("New"+componentName).Params().Op("*").Qual(tueImportPath, "ComponentInstance").Block(
...
func (g *fileGenerator) renderComponent(node *gotemplate.Node) (jen.Code, bool) {
	...
	if child.component.ContractType != "" {
		childValues = append(childValues,
			jen.Id(script.ContractFieldName()).Op(":").Id(script.ContractConstructorName(child.component.Name)).Call(),
		)
	}
	statements := []jen.Code{
	...
	for _, field := range fields {
		statements = append(statements, jen.Id("child").Dot(script.ContractFieldName()).Dot(field.Name).Op("=").Add(field.Value))
	}
	...
	component := jen.Qual(tueImportPath, "ComponentWithUpdate").Call(
		jen.Lit(node.Tag),
		jen.Func().Params().Op("*").Qual(tueImportPath, "ComponentInstance").Block(statements...),
...
func (g *fileGenerator) renderComponentUpdater(child componentBinding, fields []componentField, defaultSlot jen.Code) jen.Code {
	...
	return jen.Func().Params(jen.Id("childComp").Op("*").Qual(tueImportPath, "ComponentInstance")).Block(statements...)
}
...
func renderContractTypeExpression(typ string, imports map[string]string) (jen.Code, bool) {
	expr, err := goparser.ParseExpr(typ)
	if err != nil {
		return nil, false
	}
	return renderContractType(expr, imports)
}

func renderContractType(expr ast.Expr, imports map[string]string) (jen.Code, bool) {
	switch typed := expr.(type) {
	case *ast.Ident:
	...
	case *ast.SelectorExpr:
		...
		base, ok := renderContractType(typed.X, imports)
		if !ok {
			return nil, false
		}
	...
	case *ast.ArrayType:
		element, ok := renderContractType(typed.Elt, imports)
		if !ok || typed.Len != nil {
			return nil, false
		}
		return jen.Index().Add(element), true
	case *ast.MapType:
		key, ok := renderContractType(typed.Key, imports)
		if !ok {
			return nil, false
		}
		value, ok := renderContractType(typed.Value, imports)
		if !ok {
			return nil, false
		}
	...
	case *ast.FuncType:
		parameters, ok := renderContractFunctionFields(typed.Params, imports)
		if !ok {
			return nil, false
		}
		results, ok := renderContractFunctionFields(typed.Results, imports)
		if !ok {
			return nil, false
		}
...
func renderContractFunctionFields(fields *ast.FieldList, imports map[string]string) ([]jen.Code, bool) {
	if fields == nil {
		return nil, true
	}
	var rendered []jen.Code
	for _, field := range fields.List {
		fieldType, ok := renderContractType(field.Type, imports)
		if !ok {
			return nil, false
		}
